...
public final class PanoramaParameters {
	 ...
	 * 
	 * @param observerPosition      La position de l'observateur.
	 * @param observerElevation     L'altitude de l'observateur.
	 * @param centerAzimuth         L'azimut central du Panorama sous forme
	 *                              canonique.
	 * @param horizontalFieldOfView Le champ de vue horizontal. Entre 0 (exclu) et
	 *                              2*Pi (inclu).
	 * @param maxDistance           La profondeur maximale du Panorama. Strictement
	 *                              positive.
	 * @param width                 La largeur du Panorama, strictement supérieure à
	 *                              1.
	 * @param height                La hauteur du Panorama, strictement positive.
	 * 
	 * @throws IllegalArgumentException si l'une des conditions précitées n'est pas
	 *                                  remplie.
	 * @throws NullPointerException     si la position l'observateur est null.
	 */
	public PanoramaParameters(final GeoPoint observerPosition, final int observerElevation, final double centerAzimuth,
			final double horizontalFieldOfView, final int maxDistance, final int width, final int height) {
		this.observerPosition = requireNonNull(observerPosition, "The given observer position is null.");
		this.observerElevation = observerElevation;

		checkArgument(isCanonical(centerAzimuth), "The given azimuth is not in canonical form.");
		...
		checkArgument(0 < maxDistance, "The given maxDistance is not stricly positive.");
		this.maxDistance = maxDistance;

		checkArgument(1 < width, "The given width is not stricly superior to 1.");
		this.width = width;
	 ...
	 * 
	 * @param x L'index de pixel horizontal (valide pour le Panorama).
	 * 
	 * @return L'azimut correspondant à l'index donné.
	 * 
	 * @throws IllegalArgumentException si le pixel donné n'est pas défini dans la
	 *                                  largeur du Panorama.
	 */
	public double azimuthForX(final double x) {
		checkArgument(0 <= x && x <= width() - 1, "The given index 'x' is invalid.");
		return canonicalize(centerAzimuth() + (x - centerHorizontalPixel()) * delta());
	}

	/**
	 * Calcule l'index du pixel horizontal correspondant à l'azimut donné.
	 * 
	 * @param a L'azimut (sous forme canonique et appartenant à la zone visible).
	 * 
	 * @return L'index du pixel horizontal correspondant à l'azimut donné.
	 * 
	 * @throws IllegalArgumentException si l'azimut donné n'est pas sous forme
	 *                                  canonique ou pas défini dans le champ de vue
	 *                                  horizontal du panorma.
	 */
	public double xForAzimuth(final double a) {
		final var dist = angularDistance(centerAzimuth(), a);
	 ...
	 * 
	 * @param y L'index de pixel vertical (valide pour le Panorama).
	 * 
	 ...
	 *                                  hauteur du Panorama.
	 */
	public double altitudeForY(final double y) {
		checkArgument(0 <= y && y <= height() - 1, "The given index 'y' is invalid.");
	 ...
	 * 
	 * @param a L'altitude en radians (appartenant à la zone visible)
	 * 
	 * @return L'index du pixel vertical correspondant à l'altitude donnée.
	 * 
	 * @throws IllegalArgumentException si l'altitude n'est pas définie dans le
	 *                                  champ de vue vertical du panorma.
	 */
	public double yForAltitude(final double a) {
	 ...
	 */
	protected boolean isValidSampleIndex(final int x, final int y) {
	 ...
	 */
	protected int linearSampleIndex(final int x, final int y) {
