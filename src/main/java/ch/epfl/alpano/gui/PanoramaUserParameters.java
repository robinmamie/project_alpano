...
public final class PanoramaUserParameters implements Serializable {
	 ...
	 */
	public PanoramaUserParameters(final Map<UserParameter, Integer> map) {
		checkArgument(map.size() == UserParameter.values().length, "The given map is not valid.");
		final var tempMap = new EnumMap<>(map);
		tempMap.replaceAll(UserParameter::sanitize);
		final var limit = (int) (1
				+ MAX_VERTICAL_FIELD_OF_VIEW * (tempMap.get(WIDTH) - 1.) / tempMap.get(HORIZONTAL_FIELD_OF_VIEW));
		if (limit < tempMap.get(HEIGHT)) {
			tempMap.put(HEIGHT, limit);
		}
		this.map = unmodifiableMap(tempMap);
	}
	 ...
	 * 
	 * @param observerLongitude     La longitude de lobservateur.
	 * @param observerLatitude      La latitude de lobservateur.
	 * @param observerElevation     L'élévation de lobservateur.
	 ...
	 * 
	 * @param parameter Le paramètre utilisateur demandé.
	 * 
	 * @return La valeur du paramètrew utilisateur demandé.
	 */
	public int get(final UserParameter parameter) {
	 ...
	 * 
	 * @param s L'exposant du suréchantillonage.
	 * 
	 * @return La paramètres du Panorama.
	 */
	private PanoramaParameters panoramaParametersSet(final ToIntFunction<Integer> s) {
		return new PanoramaParameters(
				...
				observerElevation(), toRadians(centerAzimuth()), toRadians(horizontalFieldOfView()),
				maxDistance() * M_PER_KM, s.applyAsInt((width())), s.applyAsInt((height())));
	}
	...
	public boolean equals(final Object thato) {
		return thato instanceof PanoramaUserParameters that && this.map().equals(that.map());
	}
	...
	public String toString() {
		final var northOrSouth = observerLatitude() >= 0 ? 'N' : 'S';
