...
public final class PanoramaParametersBean {
	 ...
	 * 
	 * @param parameters Les paramètres utilisateurs du panorama.
	 */
	public PanoramaParametersBean(final PanoramaUserParameters parameters) {
		...
		parameters.map().forEach((u, x) -> {
			...
			properties.get(u).addListener((p, o, n) -> runLater(this::synchronizeParameters));
		});
	}
	...
	private void synchronizeParameters() {
		final var map = new EnumMap<UserParameter, Integer>(UserParameter.class);
	 ...
	 * 
	 * @param uP Le UserParameter concerné.
	 * 
