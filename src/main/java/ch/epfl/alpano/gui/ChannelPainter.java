...
public interface ChannelPainter {
	 ...
	 * 
	 * @param p Le panorama avec lequel on désire calculer ces distances.
	 * 
	...
	static ChannelPainter maxDistanceToNeighbors(Panorama p) {
		return (x, y) -> max(
				...
				max(p.distanceAt(x, y - 1, 0), p.distanceAt(x, y + 1, 0))) - p.distanceAt(x, y);
	}

	/**
	 * Permet de calculer la pente des voisins afin d'obtenir une approximation des
	 * lacs si = 0.
	 * 
	 * @param p Le panorama.
	 * 
	 ...
	 *         0.
	 */
	static ChannelPainter totalSlopeOfNeigbors(final Panorama p) {
		return (x, y) -> {
			var value = 0;
			for (var i = -FLATNESS_MARGIN; i <= FLATNESS_MARGIN; ++i) {
				for (var j = -FLATNESS_MARGIN; j <= FLATNESS_MARGIN; ++j) {
	 ...
	 * 
	 * @param n La valeur à additionner.
	 * 
	 * @return Un nouveau peintre de canal additionné avec l'argument <i>n</i>.
	 */
	default ChannelPainter add(final float n) {
	 ...
	 * 
	 * @param n La valeur à soustraire.
	 * 
	 * @return Un nouveau peintre de canal soustrait par l'argument <i>n</i>.
	 */
	default ChannelPainter sub(final float n) {
		return (x, y) -> valueAt(x, y) - n;
	}
	 ...
	 * 
	 * @param n La valeur à multiplier.
	 * 
	 * @return Un nouveau peintre de canal multiplié avec l'argument <i>n</i>.
	 */
	default ChannelPainter mul(final float n) {
		return (x, y) -> valueAt(x, y) * n;
	}
	 ...
	 * 
	 * @param n La valeur à diviser.
	 * 
	 * @return Un nouveau peintre de canal divisé par l'argument <i>n</i>.
	 */
	default ChannelPainter div(final float n) {
		return (x, y) -> valueAt(x, y) / n;
	}
	 ...
	 * 
	 * @param f Le lambda à appliquer.
	 * 
	 * @return Un nouveau peintre de canal auquel un lambda a été appliqué.
	 */
	default ChannelPainter map(final DoubleUnaryOperator f) {
	...
	/**
	 * Retourne un peintre de canal n'ayant que des valeurs entre 0 et 1. Les autres
	 * valeurs sont arrondies à la valeur valide la pluis proche.
	 * 
	 ...
	 * valeurs ne s'y trouvant pas déjà sont obtenues en gardant leur partie décimal
	 * (floorMod).
	 * 
	 * @return Le peintre de canal actualisé.
	 */
